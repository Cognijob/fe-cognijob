...
export default function RecruiterLayout() {
      ...
      <div className="flex items-center">
        ...
        {location.pathname.includes("/applicant/detail/") ? (
          <span className="flex items-center">
            <span className="mx-2 font-normal text-[18px] text-[#B4B2A9]">
              &gt;
            </span>
            <span
            ...
            <span
              ...
              onClick={() => navigate(-1)}
            >
              {activeJobTitle || "Daftar Pelamar"}
            </span>
            <span className="mx-2 font-normal text-[18px] text-[#B4B2A9]">
              &gt;
            </span>
            <span className="text-[#0B173D] font-bold text-[18px]">
              {activeApplicantId || "Detail"}
            </span>
          </span>
          ...
          segments.map((name, index) => {
            ...
            const displayName =
              breadcrumbMap[name.toLowerCase()] ||
  ...
  const renderMenuItem = (item) => {
    const isActive =
      ...
      (item.path === "/recruiter/applicants" &&
        (location.pathname.includes("/applicants/list/") ||
          location.pathname.includes("/applicant/detail/")));
    ...
    return (
      <Link
        key={item.path}
        to={item.path}
        className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all text-[14px] font-medium ${isActive ? "bg-[#1E42AC] text-white shadow-md" : "text-white/70 hover:bg-white/10 hover:text-white"}`}
      >
        <img
          src={item.icon}
          alt={item.label}
  ...
  return (
    <div
      className="flex w-full min-h-screen bg-[#FBFAFF]"
      ...
      <aside className="w-[260px] bg-[#0B173D] text-white fixed h-full flex flex-col z-50 shadow-xl">
        ...
        <nav className="flex-1 flex flex-col gap-6 overflow-y-auto px-4 py-8 scrollbar-hide">
          <div className="flex flex-col gap-1">
            <h3 className="px-4 text-[11px] font-extrabold text-[#8A95A5] uppercase tracking-widest mb-2">
              Main Menu
            </h3>
